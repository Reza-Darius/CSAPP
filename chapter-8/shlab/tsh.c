...
void eval(char *cmdline) {
  ...
  // we need to block SIGCHLD as to not get interrupted before we add the new
  // process to the job list
  ...
  if (child_pid == 0) {
    // unblocking SIGCHLD in the child
    if (sigprocmask(SIG_SETMASK, &prev, NULL))
      unix_error("sigprocmask unblock in child");

    // assign child to new process group
    if (setpgid(0, 0) < 0)
...
int builtin_cmd(char **argv) {
  if (strcmp(argv[0], "quit") == 0) {
    // do we need to wait for children? no because orphaned children
    // get cleaned up by init process
    exit(0);
  }
...
void sigint_handler(int sig) {
  ...
  // sending SIGINT to foreground process group
